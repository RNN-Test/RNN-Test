...
def run_adv(session, model, inverseDictionary, data, eval_op, verbose=False):
    ...
    for step, (x, y) in enumerate(reader.ptb_iterator(data, model.batch_size,
        ...
        guided_coverage = "hidden_state"  # could be configured
        num_neurons = model.batch_size * model.num_steps * 10
        objective2 = neuron_selection(num_neurons, guided_coverage, h_states_all, c_states_all,
        ...
        # originally predict
        fetches = grads  # model.cost, model.initial_state, model.logits, model.inputs, model.embedding, h_states_all, c_states_all
        feed_dict = {}
        feed_dict[model.input_data] = x
        feed_dict[model.targets] = y
        ...
        grads_value = session.run(fetches, feed_dict)
