...
def compute_mfcc(audio, **kwargs):
    ...
    TensorFlow so that we can differentiate through it.   #便于求导
    """
...
def get_logits(new_input, length, first=[]):
    ...
    logits, states_all, RNN_inputs = DeepSpeech.BiRNN(features, length, [0]*10)  # in DeepSpeech.py, BiRNN返回的是最后一层，为什么说是logits层

    return logits, states_all, RNN_inputs
