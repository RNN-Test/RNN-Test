...
with tf.Graph().as_default() as graph:
    ...
    with tf.Session(graph=graph) as sess:
        ...
        logits2, _, _ = DeepSpeech.BiRNN(new_input, [len(mfcc)], [0] * 10)
        ...
        for var in tf.global_variables():
            print(var.name)
            if 'stateful_rnn_cell' in var.name:
                new_name = var.name.replace('stateful_rnn_cell/', '')
                x = sess.run(var.assign(sess.run('newname/' + new_name)))
                print(x.shape)
            else:
                x = sess.run(var.assign(sess.run('newname/' + var.name)))
                print(x.shape)
